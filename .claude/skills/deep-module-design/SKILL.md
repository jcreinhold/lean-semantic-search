...
- `writing-mathematical-proofs` — when the boundary in question is a theorem-and-its-proof rather than a code module.
  The same philosophy carries over (theorem statement = interface; lemma = function); the proof-side disciplines (Tao's
  "easy to use, not easy to prove"; Lamport's hierarchical method; Leron's level test) live there.
